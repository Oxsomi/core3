...
enum Key {
	...
	Key_Delete,			Key_Help,		Key_Menu,		Key_NumLock,		Key_ScrollLock,		Key_Apps,
	...
	Key_Previous,		Key_Clear,		Key_Zoom,		Key_Enter,
	...
	Key_Numpad0,		Key_NumpadMul,	Key_NumpadAdd,
	Key_NumpadDec,		Key_NumpadDiv,	Key_NumpadSub,
...
enum KeyboardFlags {
	...
	KeyboardFlags_Insert
};

struct Error Keyboard_create(Keyboard *result);
struct Error Keyboard_free(Keyboard *dev);

//Resolves a key to a character using the current keyboard state
//Returns 0 if the key can't be resolved

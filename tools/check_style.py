...
#        Allowed in: src/platforms/unix, src/platforms/windows,
#                    src/types/container/perf/exec/main.c,
...
#   B6.  memcpy / memmove
#        Allowed in: src/types/base/buffer.c
...
BANNED_SYMBOLS: list[dict] = [
    ...
    {
        "pattern": re.compile(r'\bnew\s+\w|\bdelete\s+[^;=]'),
    ...
    # B6, memcpy / memmove
    {
        ...
        "allow_paths": ("src/types/base/buffer.c",),
    },
    # B7, memset
    {
...
POSIX_HEADERS = {
    ...
    "fcntl.h", "dirent.h", "dlfcn.h", "signal.h", "termios.h",
}
...
def check_file(
    ...
    if is_header:
        ...
        has_ifndef = any(re.match(r'^\s*#\s*ifndef\s+\w',     l) for l in lines)
        if not has_pragma:
    ...
    for lineno, line in enumerate(lines, start=1):
        ...
        # ── Banned symbols (skip pure comment lines) ──────────────────────

...
//threadId 0 is always the thread that calls JobQueue_wait (the owner); in single threaded mode it is the only context.
...
//- threadCount > 1: threadCount - 1 workers are created
...
// so "all work for X is done" cannot be observed by waiting on any single job.
//A JobGroup tracks that with a token count instead:
// register work with JobGroup_enter before pushing the jobs that will complete it,
// and release a token with JobGroup_leave as each unit finishes.
//When the last token drops the group is done - "last one out" fires the finalize
// (pushed as its own job, so its work, e.g. combining + writing an oiSH, overlaps other compiles).
...
// JobGroup_enter(g, k) before pushing k children, JobGroup_leave(g) once you finish spawning.
//This prevents the latch from hitting 0 before all children are registered.
//
//data is the shared accumulator handed to finalize;
// it is owned by finalize on the success path (finalize frees it)
// and by dataDestructor otherwise (failure, or the finalize job being discarded on shutdown).
//group must outlive all its jobs (address stable, like JobQueue).
...
//Register 'count' more tokens.
//Call before pushing the jobs that will release them.
Bool JobGroup_enter(JobGroup *group, U64 count, Error *e_rr);

//Release one token.
//When the last one drops: pushes finalize (success) or runs dataDestructor
// (if any JobGroup_fail happened, or there is no finalize).
//Returns false only on invalid usage.
Bool JobGroup_leave(JobGroup *group, Error *e_rr);

//Mark the group failed (sticky).
//Downstream should stop spawning; finalize will be skipped.

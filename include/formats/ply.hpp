...
//formats/ply.hpp
//
//C++ layer over the PLY reader. Everything but the name of the C entry point lives in mesh.hpp, since a mesh is
...
namespace oxc {
	...
	namespace ply {
		...
		[[nodiscard]] inline c::Bool read(
			...
			return mesh::read(c::PLY_read, fileBytes, types, output, info, alloc, flags, e_rr);
		}
		...
		[[nodiscard]] inline c::Bool read(
			...
			return mesh::read(
				c::PLY_read, fileBytes, types, positions, indices, attributes, triangles, info, alloc, flags, e_rr
			);
		}
	}
}

...
- `-recursion-depth <n>` supplies the ray tracing recursion limit (default 1, range 1..255).
...
- `-rtv <fmt>[,<fmt>...]` overrides the render-target formats of a live graphics pipeline (e.g. `-rtv rgba16f,r32f`), since the target format changes the pixel shader's export instructions. Formats use the OxC3 texture-format names (`rgba8`, `rgba16f`, `r32f`, ...), case-insensitive.

...
- `OxC3 isa disassemble -input <file.spv|.oiSH> -asic <asic>`: disassemble a shader to AMD ISA for `asic`, prepended with a register/resource-usage summary (VGPRs, SGPRs, LDS, scratch, spills, ISA size) parsed from RGA's analysis. Without `-output` the result is printed; with `-output <file>` it is written there.
  ...
  - DXIL has no offline path here: offline AMD ISA is derived from SPIR-V only. Disassembling DXIL would need a live AMD device (a future addition).

`-asic live` (or `live:<index>`) takes a different route from the offline amdllpc path: it creates a real Vulkan device and reads the driver's own compiled shader statistics and ISA back for the oiSH, via `VK_KHR_pipeline_executable_properties`. This is cross-vendor (any driver that implements the extension) but device- and driver-dependent, so it isn't reproducible the way the offline path is. `live` targets device 0; `live:<index>` targets a specific GPU (the command lists them first). It needs a graphics-enabled build.

- What comes back depends on the driver: register/VGPR/SGPR statistics come from any driver implementing the extension, but the disassembly **text** only comes from AMD's own driver and the open-source Mesa drivers (RADV/ANV/Turnip/NVK/PanVK, i.e. Linux); other proprietary drivers (NVIDIA, Qualcomm, ARM) return statistics only, and NVIDIA additionally returns nothing at all for ray tracing pipelines.

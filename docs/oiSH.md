...
typedef enum ESHExtension {
	...
	ESHExtension_Barycentrics				= 1 << 11,		//SV_Barycentrics (fragment); BaryCoordKHR in SPIRV
...
- Capabilities:
  ...
  - FragmentBarycentricKHR as Barycentrics.
...
- When stripping SPIRV info from one that has both DXIL and SPIRV, it will keep the reflection data it gained from merging the two. This is intentional, as this would allow you to re-gain some reflection info that is missing from DXIL and keeps the reflection data consistent across two different splits. This is what `SHFile_split` (`OxC3 file split`) does; only the things that are genuinely per binary type are removed:
  - Binaries with no compiled code for the requested type, along with the entrypoints that are left pointing at nothing. An entrypoint annotated `[[oxc::binary()]]` with only the other type disappears this way.
  - Registers the other type had on its own. That is what puts the standalone DXIL sampler, the SPIRV subpass input and the `$Globals` / push constant pair (which combine never matched, since their names differ) back in the half they came from. A register belongs to a binary type when it has a binding for it, except a SPIRV push constant, which is in no descriptor set and so is recognized by its used flag instead.
  - Everything else, including the other type's bindings on a register that survives, is carried over untouched. A combined sampler stays flagged as one on the DXIL half and a promoted sampler comparison state stays promoted on the SPIRV half, so both halves keep answering the same way.
  - Extensions are not recomputed either, so a half can still claim Bindless that it no longer needs on its own. `SHBinaryIdentifier_equals` already ignores Bindless, UnboundArraySize and DynamicSamplers for exactly this reason.
  - A reflection-only oiSH is split per register alone. Which binary type a binary was for is expressed by which of its buffers carry code (ESHBinaryFlags is derived from that when writing), so stripping a file to reflection takes that with it and every binary and entrypoint has to be kept. The registers still carry their per binary type bindings, so each half is still the reflection that backend would have had.

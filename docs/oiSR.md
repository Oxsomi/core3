...
## Reflecting a source that does not compile

An editor wants an outline for the file as it is being typed, which is most of the time a file that does not compile. `CompilerSettings::reflectAllowErrors` asks `Compiler_reflect` for whatever survived the frontend's error recovery instead of refusing on the first diagnostic; it reaches DXC as `-reflect-allow-errors`. Only reflection takes it, so nothing that produces an oiSH can consume a partial tree.

What survives is the frontend's decision and moves with it, so no particular node is promised. What is promised is that the SRFile is structurally sound whatever the input was: parents in range and preceding their children, the symbol array parallel to the node array, and every keyed record naming a node that exists. A recovered node can carry a placeholder type, since an undefined type resolves to `int`, so a type record says how it is shaped rather than that it is right. `src/shader_compiler/test/test_shader_compiler_reflect_errors.c` holds that contract against a set of half-written sources, and names per case the declaration that has to survive.

In practice a file keeps its declarations through an undeclared identifier, an unknown type on a member or a parameter, an unclosed brace, a half-typed member access, a call to nothing, a duplicate definition and a struct that contains itself. The one shape that yields nothing is an include that does not resolve: that is fatal to preprocessing, so nothing after it is parsed and the tree comes back as its root alone.

A struct that contains itself, or derives from itself, only exists in a source that does not compile. It reflects as a type with no members and no base rather than being followed round the cycle, which is the same shape an opaque record has.

...
static void Test_streamedTail(
	...
	if(!Test_assert(t, name, dev.createBufferStream(
	...
	if(!Test_assert(t, "setBigInput", table.setByName("input", tailDesc, 0, true, e_rr)))
		return;

	gfx::CommandList bigList;

	if(!Test_assert(t, "bigList", dev.createCommandList(2 * c::KIBI, 32, 16, bigList, true, e_rr)))
		return;
...

	{
		...
		Test_assert(t, "bigHeap", scope.bindDescriptorHeap(heap, e_rr));
		Test_assert(t, "bigTable", scope.bindDescriptorTable(table, e_rr));
		Test_assert(t, "bigPipeline", scope.setComputePipeline(pipeline, e_rr));
	...
	if(!gfxtest::submitAndWait(t, dev, bigList) || !gfxtest::pullBuffer(t, dev, emptyList, output))
		return;

	const TestStreamElem *v = (const TestStreamElem*) output.data()->cpuData.ptr;
...
extern "C" void Test_graphicsBufferStream(oxc::c::Test *t, oxc::c::GraphicsDeviceRef *deviceRef) {
...

	{
		...
		Test_assert(t, "cpuBackedRefused", !dev.createBufferStream(
			...
			"Stream CPUBacked", (c::StreamRef*) source, sizeof(elems), refused, false, nullptr, nullptr
		));
		...
		Test_assert(t, "nullStreamRefused", !dev.createBufferStream(
			c::EDeviceBufferUsage_None, c::EGraphicsResourceFlag_ShaderRead,
			"Stream null", nullptr, sizeof(elems), refused, false, nullptr, nullptr
		));
	}
	...
	if (gfxtest::submitAndWait(t, dev, commandList))
		if (gfxtest::pullBuffer(t, dev, emptyList, output)) {
			...
			const TestStreamElem *values = (const TestStreamElem*) output.data()->cpuData.ptr;
	...
	if(!Test_assert(t, "markSecondHalf", c::DeviceBufferRef_markDirty(
		input.handle(), sizeof(elems) / 2, sizeof(elems) / 2, e_rr
	)))
		return;
	...
	// for an incoherent heap's flush. A copy would be the bytes onto themselves.

	gfx::DeviceBuffer mapped;

	if(!Test_assert(t, "mappedCreate", dev.createBuffer(
		c::EDeviceBufferUsage_None,
		(c::EGraphicsResourceFlag) (c::EGraphicsResourceFlag_ShaderRead | c::EGraphicsResourceFlag_CPUAllocatedBit),
		"Mapped input", sizeof(elems), mapped, nullptr, e_rr
	)))
		return;
	...
	Test_assert(t, "setMappedInput", table.setByName("input", mappedDesc, 0, false, e_rr));

	gfx::CommandList mappedList;

	if(!Test_assert(t, "mappedListCreate", dev.createCommandList(2 * c::KIBI, 32, 16, mappedList, true, e_rr)))
		return;
...

	{
		...
		Test_assert(t, "mappedBindHeap", scope.bindDescriptorHeap(heap, e_rr));
		Test_assert(t, "mappedBindTable", scope.bindDescriptorTable(table, e_rr));
		Test_assert(t, "mappedBindPipeline", scope.setComputePipeline(pipeline, e_rr));
	...
	if (gfxtest::submitAndWait(t, dev, mappedList))
		if (gfxtest::pullBuffer(t, dev, emptyList, output)) {
	...
	{
		c::GraphicsDeviceRef *dr = dev.handle();
		...
		if(Test_assert(t, "bigBacking", c::Buffer_createEmptyBytes(bigLen, dev.alloc(), &backing, e_rr))) {
			...
			if(Test_assert(t, "bigStream", c::MemoryStream_createFromBufferRegion(
				...
				c::EMemoryStreamFlags_None, &streamType, &bigStream, e_rr
			))) {

				Test_streamedTail(
					t, dev, heap, table, pipeline, output, emptyList,
					(c::StreamRef*) bigStream, bigLen, "Big stream", "bigTailUploaded"
				);

				c::RefPtr_dec((c::RefPtr**) &bigStream);
			}

			const c::RefPtrType fileHandleType = c::FileHandle_makeType(dev.alloc());
			const c::RefPtrType fileStreamType = c::FileStream_makeType(dev.alloc());
			const c::CharString bigPath = c::CharString_createRefCStrConst("graphics_test_big_stream.bin");

			c::File_remove(&bigPath, 1 * c::SECOND, dev.alloc(), NULL);
			...
			if(Test_assert(t, "bigFileWrite", c::File_write(
				...
				if(Test_assert(t, "bigFileStream", c::File_openStream(
					...
					Test_streamedTail(
						t, dev, heap, table, pipeline, output, emptyList,
						fileStream, bigLen, "Big file stream", "bigFileTailUploaded"
					);
			...
			c::Buffer_free(&backing, dev.alloc());
		}
	}
...

	{
		...
		const c::U64 oddLen = sizeof(elems) + 3;        //Three bytes into the 65th element
		...
		if(Test_assert(t, "oddStream", c::MemoryStream_createFromBufferRegion(
			...
			c::EMemoryStreamFlags_None, &streamType, &oddStream, e_rr
		))) {

			if(Test_assert(t, "oddBuffer", dev.createBufferStream(
				...
				const c::Descriptor oddDesc = c::Descriptor_buffer(oddBuf.handle(), 0, sizeof(elems), NULL, 0);

				//maintainRef for the same reason the tail leg needs it: this buffer goes out of scope while
				// the table is still holding its descriptor.

				if(Test_assert(t, "setOddInput", table.setByName("input", oddDesc, 0, true, e_rr)))
					if(gfxtest::submitAndWait(t, dev, commandList))
						if(gfxtest::pullBuffer(t, dev, emptyList, output)) {
...

	{
		...
		if(Test_assert(t, "oneShotStream", c::MemoryStream_createFromBufferRegion(
			c::Buffer_createRefConst(elems, sizeof(elems)), 0, sizeof(elems),
			c::EMemoryStreamFlags_None, &streamType, &once, e_rr
		))) {

			if(Test_assert(t, "oneShotCreate", dev.createBufferStream(
				c::EDeviceBufferUsage_None, c::EGraphicsResourceFlag_ShaderRead,
				"One shot", (c::StreamRef*) once, sizeof(elems), oneShot, false, nullptr, e_rr
			))) {
				...
				if(Test_assert(t, "oneShotSubmit", gfxtest::submitAndWait(t, dev, emptyList))) {
	...
	if(!Test_assert(t, "jobQueue", c::JobQueue_create(4, dev.alloc(), &queue, e_rr)))
		return;
	...
	c::Bool madeStreams =
		Test_assert(t, "sharedStream", c::MemoryStream_create(
			big, c::EMemoryStreamFlags_None, &streamType, &shared, e_rr
		)) &&
		Test_assert(t, "resizableStream", c::MemoryStream_create(
			big, c::EMemoryStreamFlags_IsResizable, &streamType, &resizable, e_rr
		));
	...
	if(madeStreams) {
		...
		c::Bool madeBuffers =
			Test_assert(t, "sharedBuffer", dev.createBufferStream(
				c::EDeviceBufferUsage_None, c::EGraphicsResourceFlag_ShaderRead,
				"Fan out shared", sharedRef, big, sharedBuf, true, nullptr, e_rr
			)) &&
			Test_assert(t, "resizableBuffer", dev.createBufferStream(
				c::EDeviceBufferUsage_None, c::EGraphicsResourceFlag_ShaderRead,
		...
		if(madeBuffers) {
			...
			c::GraphicsDeviceRef *devRef = dev.handle();
			...
			Test_assert(t, "lendSucceeds", dev.submitJob({ &emptyList }, {}, &queue, 0, 0, e_rr));
			...
			Test_assert(t, "fanOutDrained", dev.wait(e_rr));
		}
	}

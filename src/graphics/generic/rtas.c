...
Bool RTAS_validateDeviceBuffer(DeviceData *bufPtr, Error *e_rr) {
	...
	if(bufPtr->offset >= totalLen)
		retError(clean, Error_nullPointer(0, "BLAS_validateDeviceBuffer()::bufPtr->offset out of bounds"));

	if(!bufPtr->len)
		bufPtr->len = totalLen - bufPtr->offset;

	if(bufPtr->offset >> 48 || bufPtr->len >> 48)
		retError(clean, Error_unsupportedOperation(1, "BLAS_validateDeviceBuffer()::buf.len or offset out of bounds"));

	U64 len = bufPtr->offset + bufPtr->len;

	if(len > totalLen)
		retError(clean, Error_invalidParameter(1, 0, "BLAS_validateDeviceBuffer()::buf is out of bounds"));

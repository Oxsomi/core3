...
Error validateBlend(EBlend blend, Bool hasDualSrcBlend) {
	...
	if(blend >= EBlend_Count)
		return Error_invalidEnum(0, (U64)blend, EBlend_Count, "validateBlend()::blend is out of bounds");

	if(!hasDualSrcBlend && blend >= EBlend_Src1ColorExt && blend <= EBlend_InvSrc1AlphaExt)
		return Error_unsupportedOperation(0, "validateBlend()::blend is unsupported since hasDualSrcBlend is false");
...
Error GraphicsDeviceRef_createPipelinesGraphics(
	...
	if(pipelines->ptr)
		return Error_invalidParameter(
			4, 0, "GraphicsDeviceRef_createPipelinesGraphics()::pipelines->ptr is non zero, indicating a possible memleak"
		);
	...
	for(U64 i = 0; i < infos->length; ++i) {
		...
		if (!(device->info.capabilities.features & EGraphicsFeatures_DirectRendering)) {
			...
			if((info->renderPass != NULL) == (info->attachmentCountExt || info->depthFormatExt))
				return Error_invalidOperation(
					4, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i] should pick either directRendering or renderPass"
				);
		}
		...
		for (U64 j = countStart; j < countStart + info->stageCount; ++j) {
			...
			if(stage.stageType >= EPipelineStage_Count)
				return Error_invalidOperation(
					21, "GraphicsDeviceRef_createPipelinesGraphics()::stages[j].stageType is invalid"
				);

			if((stageFlags >> stage.stageType) & 1)
				return Error_alreadyDefined(
					0, "GraphicsDeviceRef_createPipelinesGraphics()::stages[j].stageType is duplicate for infos[i]"
				);
		...
		for(U32 j = 0; j < 16; ++j) {
			...
			if(attrib.format >= ETextureFormatId_Count)
				return Error_invalidOperation(
					8, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i].vertexLayout.attributes[j].format is invalid"
				);
			...
			if(offset + size > stride)
				return Error_outOfBounds(
					...
					"GraphicsDeviceRef_createPipelinesGraphics()::infos[i].vertexLayout.attributes[j].offset is out of bounds"
				);
		}
		...
		if(
			...
			return Error_unsupportedOperation(
				5, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i] rasterizer uses depth bias but depthBias is disabled"
			);
		...
		if(
			...
			return Error_unsupportedOperation(
				4, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i] requires wireframe extension, which isn't supported"
			);
		...
		if (
			...
			return Error_unsupportedOperation(
				6, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i] stencilTest is off, but stencil params were set"
			);

		if(info->depthStencil.depthCompare && !(info->depthStencil.flags & EDepthStencilFlags_DepthTest))
			return Error_unsupportedOperation(
				7, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i] depthTest is off but depthCompare was set"
			);
		...
		if(info->depthStencil.stencilCompare >= ECompareOp_Count)
			return Error_invalidOperation(
				12, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i].depthStencil.stencilCompare is out of bounds"
			);

		if(info->depthStencil.depthCompare >= ECompareOp_Count)
			return Error_invalidOperation(
				13, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i].depthStencil.depthCompare is out of bounds"
			);

		if(info->depthStencil.stencilFail >= EStencilOp_Count)
			return Error_invalidOperation(
				14, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i].depthStencil.stencilFail is out of bounds"
			);

		if(info->depthStencil.stencilPass >= EStencilOp_Count)
			return Error_invalidOperation(
				15, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i].depthStencil.stencilPass is out of bounds"
			);

		if(info->depthStencil.stencilDepthFail >= EStencilOp_Count)
			return Error_invalidOperation(
				16, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i].depthStencil.stencilDepthFail is out of bounds"
			);
		...
		for(U64 j = 0; j < 8; ++j) {
			...
			if(attachj.blendOp >= EBlendOp_Count)
				return Error_invalidOperation(
					17, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i].blendState.attachments[j].blendOp out of bounds"
				);
		...
		for(U32 j = 0; j < 8; ++j)
			if(info->attachmentFormatsExt[j] >= ETextureFormatId_Count)
				return Error_invalidOperation(
					22, "GraphicsDeviceRef_createPipelinesGraphics()::infos[i].attachmentFormatsExt[j] out of bounds"
				);
	}
...
Error GraphicsDeviceRef_createPipelineRaytracing(
	...
	if(!binaries->length || !stages.length || !infos.length)
		return Error_invalidParameter(
			1, 0, "GraphicsDeviceRef_createPipelinesRaytracing()::binaries, stages and infos should be of length > 0"
		);
	...
	if(pipelines->ptr)
		return Error_invalidParameter(
			3, 0, "GraphicsDeviceRef_createPipelinesRaytracing()::pipelines->ptr is non zero, indicating a possible memleak"
		);
	...
	for(U64 i = 0; i < binaries->length; ++i)
		if(!Buffer_length(binaries->ptr[i]))
			return Error_invalidParameter(1, 0, "GraphicsDeviceRef_createPipelinesRaytracing()::binaries[i] is required");
	...
	for(U64 i = 0; i < stages.length; ++i)
		if(stages.ptr[i].stageType < EPipelineStage_RtStart || stages.ptr[i].stageType > EPipelineStage_RtEnd)
			return Error_invalidParameter(1, 0, "GraphicsDeviceRef_createPipelinesRaytracing()::stageType is not RT capable");
	...
	for(U64 j = 0; j < infos.length; ++j) {
		...
		if(info.flags >> EPipelineRaytracingFlags_Count)
			return Error_invalidParameter(1, 0, "GraphicsDeviceRef_createPipelinesRaytracing()::info[i].flags is invalid");
		...
		if(info.binaries.ptr || info.groups.length)
			return Error_invalidParameter(
				1, 0, "GraphicsDeviceRef_createPipelinesRaytracing()::info[i].binaries and groups should be NULL"
			);
		...
		for (U64 i = 0; i < info.groupCount; ++i) {
			...
			if(group.intersection != U32_MAX && (info.flags & EPipelineRaytracingFlags_SkipAABBs))
				return Error_invalidParameter(
					1, 0, 
					"GraphicsDeviceRef_createPipelinesRaytracing()::groups[i].intersection is disallowed if aabbs are skipped"
				);

			if(group.anyHit == U32_MAX && (info.flags & EPipelineRaytracingFlags_NoNullAnyHit))
				return Error_invalidParameter(
					1, 0, 
					"GraphicsDeviceRef_createPipelinesRaytracing()::groups[i].anyHit is null, but NoNullAnyHit is used"
				);

			if(group.closestHit == U32_MAX && (info.flags & EPipelineRaytracingFlags_NoNullClosestHit))
				return Error_invalidParameter(
					1, 0, 
					"GraphicsDeviceRef_createPipelinesRaytracing()::groups[i].closestHit is null, but NoNullClosestHit is used"
				);
			...
			if(group.anyHit != U32_MAX && stages.ptr[group.anyHit + i].stageType != EPipelineStage_AnyHitExt)
				return Error_invalidParameter(
					1, 0,
					"GraphicsDeviceRef_createPipelinesRaytracing()::stages[i].anyHit pointed to the wrong pipeline stage"
				);

			if(group.closestHit != U32_MAX && stages.ptr[group.closestHit + i].stageType != EPipelineStage_ClosestHitExt)
				return Error_invalidParameter(
					1, 0,
					"GraphicsDeviceRef_createPipelinesRaytracing()::stages[i].closestHit pointed to the wrong pipeline stage"
				);

			if(group.intersection != U32_MAX && stages.ptr[group.intersection + i].stageType != EPipelineStage_IntersectionExt)
				return Error_invalidParameter(
					1, 0,
					"GraphicsDeviceRef_createPipelinesRaytracing()::stages[i].intersection pointed to the wrong pipeline stage"
				);
		}
	...
	if(binaries->length != binaryCounter)
		return Error_invalidParameter(1, 0, "GraphicsDeviceRef_createPipelinesRaytracing()::binaries has leftover elements");

	if(stages.length != stageCounter)
		return Error_invalidParameter(1, 0, "GraphicsDeviceRef_createPipelinesRaytracing()::stages has leftover elements");

	if(groups.length != groupCounter)
		return Error_invalidParameter(1, 0, "GraphicsDeviceRef_createPipelinesRaytracing()::groups has leftover elements");
	...
	if(!(dev->info.capabilities.features & EGraphicsFeatures_RayPipeline))
		return Error_invalidParameter(
			1, 0, "GraphicsDeviceRef_createPipelinesRaytracing() can't be called if RayPipeline isn't supported"
		);

	if(anyMotionBlurExt && !(dev->info.capabilities.features & EGraphicsFeatures_RayMotionBlur))
		return Error_invalidParameter(
			1, 0, "GraphicsDeviceRef_createPipelinesRaytracing() can't enable motion blur if the feature isn't supported"
		);

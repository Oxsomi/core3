...
#define _CastFromU(type, toBits, ...) {																	\
																										\
	if(!res)																							\
		return Error_nullPointer(1, "_CastFromU()::res is required");									\
																										\
	__VA_ARGS__																							\
																										\
	if(v > type##_MAX)																					\
		return Error_overflow(0, toBits, (U64) type##_MAX, "_CastFromU()::v out of bounds");			\
																										\
	*res = (type) v;																					\
	return Error_none();																				\
}
...
#define _CastFromI(type, castType, toBits, ...) _CastFromU(type, toBits, __VA_ARGS__ 					\
	if(v < type##_MIN)																					\
		return Error_underflow(0, toBits, (castType) type##_MIN, "_CastFromI()::v is out of bounds");	\
)

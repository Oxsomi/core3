...
//Several records reference a node that has to be a struct or a union: a type's definition and base class, and
//the source side of an interface edge.

static Bool srReferencesStructOrUnion(const SRFile *srFile, U32 nodeId) {

	if(nodeId >= srFile->nodes.length)
		return false;

	const U8 type = srFile->nodes.ptr[nodeId].type;
	return type == ESRNodeType_Struct || type == ESRNodeType_Union;
}
...
Bool SRFile_finalize(SRFile *srFile, const Allocator *alloc, Error *e_rr) {
	...
	//Every reference a walker follows has to land inside the pool it names, whether the file came off disk or
	//straight out of the reflector, so the rules live here rather than in the reader alone.

	const U64 nameCount = srFile->names.entryStrings.length;
	...
	for(U64 i = 0; i < srFile->nodes.length; ++i) {
		...
		if(node.type >= ESRNodeType_Count)
			retError(clean, Error_invalidState(0, "SRFile_finalize() node had invalid type"));

		if(node.interpolation >= ESRInterpolation_Count)
			retError(clean, Error_invalidState(0, "SRFile_finalize() node had invalid interpolation"));

		if(node.flags & ESRNodeFlag_Invalid)
			retError(clean, Error_invalidState(0, "SRFile_finalize() node had invalid flags"));

		if(node.nameId != U32_MAX && node.nameId >= nameCount)
			retError(clean, Error_invalidState(0, "SRFile_finalize() node.nameId out of bounds"));

		if(node.semanticId != U32_MAX && node.semanticId >= nameCount)
			retError(clean, Error_invalidState(0, "SRFile_finalize() node.semanticId out of bounds"));
		...
		if(node.parent != U32_MAX && node.parent >= i)
			retError(clean, Error_invalidState(0, "SRFile_finalize() node.parent must reference an earlier node (or be root)"));

		//Direct children are emitted contiguously, so the child block must fit within the node array.

		if((U64)i + node.childCount > srFile->nodes.length)
			retError(clean, Error_invalidState(0, "SRFile_finalize() node.childCount out of bounds"));

		if(node.fwdBckDeclareNode != U32_MAX && node.fwdBckDeclareNode >= srFile->nodes.length)
			retError(clean, Error_invalidState(0, "SRFile_finalize() node.fwdBckDeclareNode out of bounds"));
		...
		if(
			...
			retError(clean, Error_invalidState(0, "SRFile_finalize() forward-declare flag on a non-declarable node type"));

		if((node.flags & ESRNodeFlag_HasReturn) && node.type != ESRNodeType_Function)
			retError(clean, Error_invalidState(0, "SRFile_finalize() HasReturn flag on a non-function node"));

		if((node.flags & ESRNodeFlag_ParamReturn) && node.type != ESRNodeType_Parameter)
			retError(clean, Error_invalidState(0, "SRFile_finalize() return-value flag on a non-parameter node"));

		if((node.flags & (ESRNodeFlag_ParamIn | ESRNodeFlag_ParamOut)) && node.type != ESRNodeType_Parameter)
			retError(clean, Error_invalidState(0, "SRFile_finalize() parameter-direction flag on a non-parameter node"));

		if(node.annotationCount) {

			if(node.annotationStart == U32_MAX || (U64)node.annotationStart + node.annotationCount > srFile->annotations.length)
				retError(clean, Error_invalidState(0, "SRFile_finalize() node annotation range out of bounds"));
		}
	}
	...
	for(U64 i = 0; i < srFile->nodes.length; ++i) {
		...
		if(j == i)
			retError(clean, Error_invalidState(0, "SRFile_finalize() node links to itself as a forward/backward declaration"));

		SRNode other = srFile->nodes.ptr[j];

		if(other.type != node.type)
			retError(clean, Error_invalidState(0, "SRFile_finalize() forward/backward declaration type mismatch"));

		if(other.nameId != node.nameId)        //Both are U32_MAX when symbols are absent, so this still holds
			retError(clean, Error_invalidState(0, "SRFile_finalize() forward/backward declaration name mismatch"));
		...
		if(isFwd) {
			...
			if(j <= i || otherIsFwd)
				retError(clean, Error_invalidState(
					0, "SRFile_finalize() forward declaration must point forward to its definition"
				));

			if(node.annotationCount)
				retError(clean, Error_invalidState(0, "SRFile_finalize() forward declaration must not carry annotations"));

			if(node.type != ESRNodeType_Function && node.childCount)
				retError(clean, Error_invalidState(
					0, "SRFile_finalize() non-function forward declaration must have no children"
				));
		}
		...
		//A definition's back-link points backward to its forward declaration
			...
			retError(clean, Error_invalidState(
				0, "SRFile_finalize() definition back-link must point to an earlier forward declaration"
			));
	}

	//Validate symbol references

	for(U64 i = 0; i < srFile->symbols.length; ++i)
		if(srFile->symbols.ptr[i].fileNameId != U32_MAX && srFile->symbols.ptr[i].fileNameId >= nameCount)
			retError(clean, Error_invalidState(0, "SRFile_finalize() symbol.fileNameId out of bounds"));

	//Validate annotation references

	for(U64 i = 0; i < srFile->annotations.length; ++i)
		if(srFile->annotations.ptr[i].nameId >= nameCount)
			retError(clean, Error_invalidState(0, "SRFile_finalize() annotation.nameId out of bounds"));

	//Validate register references (must point at a Register node, with in-range enum fields)

	for(U64 i = 0; i < srFile->registers.length; ++i) {

		SRRegister reg = srFile->registers.ptr[i];

		if(reg.nodeId >= srFile->nodes.length || srFile->nodes.ptr[reg.nodeId].type != ESRNodeType_Register)
			retError(clean, Error_invalidState(0, "SRFile_finalize() register.nodeId doesn't reference a Register node"));
		...
		if(
			...
			retError(clean, Error_invalidState(0, "SRFile_finalize() register had an invalid type/dimension/returnType"));

		if((reg.arrayDimStart != U32_MAX) != (reg.arrayDimCount >= 2))
			retError(clean, Error_invalidState(0, "SRFile_finalize() register arrayDim start/count disagree"));

		if(reg.arrayDimStart != U32_MAX && (U64) reg.arrayDimStart + reg.arrayDimCount > srFile->arrayDims.length)
			retError(clean, Error_invalidState(0, "SRFile_finalize() register.arrayDim range out of bounds"));
	}

	//Validate enum value references (must point at an EnumValue node, with an in-range underlying type)

	for(U64 i = 0; i < srFile->enumValues.length; ++i) {

		SREnumValue ev = srFile->enumValues.ptr[i];

		if(ev.nodeId >= srFile->nodes.length || srFile->nodes.ptr[ev.nodeId].type != ESRNodeType_EnumValue)
			retError(clean, Error_invalidState(0, "SRFile_finalize() enumValue.nodeId doesn't reference an EnumValue node"));
	...
	for(U64 i = 0; i < srFile->types.length; ++i) {
		...
		SRType ty = srFile->types.ptr[i];

		if(ty.nodeId >= srFile->nodes.length)
			retError(clean, Error_invalidState(0, "SRFile_finalize() type.nodeId out of bounds"));

		U8 nt = srFile->nodes.ptr[ty.nodeId].type;
		...
		if(ty.typeClass >= ESRTypeClass_Count)
			retError(clean, Error_invalidState(0, "SRFile_finalize() type had an invalid typeClass"));

		if(ty.typeNameId != U32_MAX && ty.typeNameId >= nameCount)
			retError(clean, Error_invalidState(0, "SRFile_finalize() type.typeNameId out of bounds"));

		if(ty.displayNameId != U32_MAX && ty.displayNameId >= nameCount)
			retError(clean, Error_invalidState(0, "SRFile_finalize() type.displayNameId out of bounds"));

		//Go-to-definition + base class must reference a Struct/Union node (or be absent)

		if(ty.defNodeId != U32_MAX && !srReferencesStructOrUnion(srFile, ty.defNodeId))
			retError(clean, Error_invalidState(0, "SRFile_finalize() type.defNodeId doesn't reference a Struct/Union node"));

		if(ty.baseNodeId != U32_MAX && !srReferencesStructOrUnion(srFile, ty.baseNodeId))
			retError(clean, Error_invalidState(0, "SRFile_finalize() type.baseNodeId doesn't reference a Struct/Union node"));

		if((ty.arrayDimStart != U32_MAX) != (ty.arrayDimCount >= 2))
			retError(clean, Error_invalidState(0, "SRFile_finalize() type arrayDim start/count disagree"));

		if(ty.arrayDimStart != U32_MAX && (U64) ty.arrayDimStart + ty.arrayDimCount > srFile->arrayDims.length)
			retError(clean, Error_invalidState(0, "SRFile_finalize() type.arrayDim range out of bounds"));
	}

	//Interface edges must go from a Struct/Union node to an Interface node

	for(U64 i = 0; i < srFile->interfaces.length; ++i) {

		SRInterface itf = srFile->interfaces.ptr[i];

		if(!srReferencesStructOrUnion(srFile, itf.nodeId))
			retError(clean, Error_invalidState(0, "SRFile_finalize() interface.nodeId doesn't reference a Struct/Union node"));

		if(
			itf.interfaceNodeId >= srFile->nodes.length ||
			srFile->nodes.ptr[itf.interfaceNodeId].type != ESRNodeType_Interface
		)
			retError(clean, Error_invalidState(
				0, "SRFile_finalize() interface.interfaceNodeId doesn't reference an Interface node"
			));
	}

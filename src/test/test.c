...
int main() {
	...
	//Same key and same IV, so the start is always the same.
			...
			for (U64 k = 0; k < sizeof(blockSizeHints) / sizeof(blockSizeHints[0]); ++k)
				for (U64 i = 0; i < sizeof(sizes) / sizeof(sizes[0]); ++i) {
					...
					if(!m) {
						...
						gotoIfError3(clean, Buffer_unsetAllBits(unit, e_rr));
						...
						Buffer test = Buffer_createRefConst(validationData, U64_min(sizeof(validationData), siz));
						...
						Buffer newZero = Buffer_createRef(full.ptrNonConst + siz, siz);

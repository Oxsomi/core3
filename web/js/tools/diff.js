...
/* Diffed disassembly reads like the SPV/DXIL tabs do; the add/del verdict is the line background. */
const hlAsm = l => window.OxUtil.highlightAsm(l) || "&nbsp;";

/* Nothing awaits this render, so a failure inside it has to show in the pane: an unhandled rejection
 * would leave the body silently empty, which reads as "the diff doesn't work". */
function renderBinaryDiff() {
  renderBinaryDiffInner().catch(err => {
    $("#bdSummary").innerHTML = `<span class="text-danger">binary diff failed: ${esc(err.message)}</span>`;
    $("#bdBody").innerHTML = "";
  });
}

async function renderBinaryDiffInner() {
  ...
  const flushEq = (before) => {
    ...
    if (!changesOnly || eqRun.length <= CTX * 2 + 1) {
      html += eqRun.map(l => `<span class="ln ctx"> ${hlAsm(l)}</span>`).join("");
    } else {
      const head = before ? [] : eqRun.slice(0, CTX);
      const tail = eqRun.slice(-CTX);
      html += head.map(l => `<span class="ln ctx"> ${hlAsm(l)}</span>`).join("");
      html += `<span class="ln skip">··· ${eqRun.length - head.length - tail.length} unchanged line(s) ···</span>`;
      html += tail.map(l => `<span class="ln ctx"> ${hlAsm(l)}</span>`).join("");
    }
    eqRun = [];
  };
  let first = true;
  for (const op of ops) {
    ...
    if (op.t === "-") html += `<span class="ln del">-${hlAsm(op.a)}</span>`;
    else html += `<span class="ln add">+${hlAsm(op.b)}</span>`;
  }
  /* trailing context */
  if (eqRun.length) {
    if (!changesOnly || eqRun.length <= CTX) html += eqRun.map(l => `<span class="ln ctx"> ${hlAsm(l)}</span>`).join("");
    else {
      html += eqRun.slice(0, CTX).map(l => `<span class="ln ctx"> ${hlAsm(l)}</span>`).join("");

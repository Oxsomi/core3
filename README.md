...
For per-module maturity, see [STATUS.md](STATUS.md). For how the modules fit together (and the error-handling idiom used everywhere), see [ARCHITECTURE.md](ARCHITECTURE.md).
...
One subtree differs: the shader playground frontend in [web/](web/README.md) is MIT licensed (see
[web/LICENSE](web/LICENSE)), as a demo of what the framework can do. The compiled OxC3 wasm module it
runs against stays under this repository's licensing, with an additional permission for serving it with
that frontend; web/README.md's License section has the exact wording.
